                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <div class="card-title">ข้อมูลจำนวนสัตว์เลี้ยงที่ไม่ได้รับการฉีดวัคซีน ในวันที่ :</div>
                                ...
                                <div class="card-body">
                                    ...
                                    <div id="petTableContainer" class="mt-4">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <h4>สัตว์เลี้ยงที่ไม่ได้รับการฉีดวัคซีน</h4>
                                                <div id="notReceivedTableContainer">
...
$(document).ready(function() {
    function loadTableData(day) {
        $.ajax({
            url: 'fetch_table_day_not.php',
            type: 'POST',
            data: {
                day: day
            },
            success: function(response) {
                // Assume response contains HTML for the table with not received data
                $('#notReceivedTableContainer').html(response);
    ...
    function exportToExcel() {
        ...
        // Export not received table
        var notReceivedTable = $('#notReceivedTable').DataTable().rows().nodes().to$();
        var notReceivedData = [];
        $('#notReceivedTable thead tr').each(function() {
            var headers = [];
            $(this).find('th').each(function() {
                headers.push($(this).text());
            });
            notReceivedData.push(headers);
        });

        notReceivedTable.each(function() {
            var row = [];
            $(this).find('td').each(function() {
                row.push($(this).text());
            });
            notReceivedData.push(row);
        });

        var ws1 = XLSX.utils.aoa_to_sheet(notReceivedData);
        XLSX.utils.book_append_sheet(wb, ws1, 'Not Received Vaccinations');

        // Save the Excel file
        var formattedDate = new Date(selectedDay).toISOString().slice(0, 10); // Convert date to YYYY-MM-DD format
        XLSX.writeFile(wb, `ข้อมูลสถิติจำนวนสัตว์ที่ไม่ได้รับวัคซีนประจำวัน_${formattedDate}.xlsx`);
    }
    ...
    $('#printPDF').click(function() {
        ...
        doc.setFontSize(20);
        doc.text(15, 15, "ข้อมูลสถิติจำนวนสัตว์ที่ไม่ได้รับวัคซีนประจำวัน วันที่: " + selectedDay);
        ...
        setTimeout(function() {
            ...
            var formattedDate = new Date(selectedDay).toISOString().slice(0, 10); // Convert date to YYYY-MM-DD format
            doc.save(`ข้อมูลสถิติจำนวนสัตว์ที่ไม่ได้รับวัคซีนประจำวัน_${formattedDate}.pdf`);
